...
const summaryCards = [
  { label: "Waiting actions", value: "2", note: "From Add action - pending inspection" },
  { label: "Waiting objectives", value: "2", note: "From Add operational objectives - pending inspection" },
  { label: "Accepted (30 days)", value: "18", note: "No further edits requested" },
  { label: "Returned for edits", value: "6", note: "Submitter notified with your notes" },
]
    ...
    <div className="min-w-0 flex-1 overflow-x-hidden bg-background p-4 sm:p-6 lg:p-8">
        <header className="mb-6 flex flex-col gap-4 sm:mb-8 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-5 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Inspection and approval</p>
            <h1 className="mt-1 text-2xl font-bold tracking-tight sm:text-3xl">Welcome, Sara</h1>
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              New actions from <span className="font-medium text-foreground">Add action</span> and objectives from{" "}
              <span className="font-medium text-foreground">Add operational objectives</span> appear in your queue for review
              before they are finalized in the plan.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2 lg:items-start">
          ...
          <Card className="flex min-h-[20rem] flex-col ring-1 ring-border/60 lg:min-h-[28rem]">
            <CardHeader>
              ...
              <CardDescription>Open an item to inspect the full submission and record your decision.</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-1 flex-col">
              ...
              <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:gap-3">
                <Link
                  to="/action-queue"
                  className="inline-flex flex-1 items-center justify-center rounded-full border border-border py-3 text-sm font-semibold text-foreground transition hover:bg-accent"
                >
                  View action queue
                </Link>
                <Link
